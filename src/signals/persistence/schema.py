...
for_you_sentence = sa.Table(
    ...
    sa.Column("attempt_day", sa.Date, index=True),
    ...
    sa.Column("model_fit", sa.String(16)),
...
prospect_send_request = sa.Table(
    ...
    sa.Column("claimed_by", sa.String(320)),
...
prospect_send_item = sa.Table(
    ...
    sa.Index("ix_prospect_send_item_request_position", "request_id", "position"),
)
...
# Milo Mail's preparatory census is program-scoped. It contains public company
# facts and opaque references only; contact details remain in Kivou's stores.
acquisition_census_run = sa.Table(
    "acquisition_census_run", METADATA,
...
acquisition_census_partition = sa.Table(
    "acquisition_census_partition", METADATA,
    ...
    sa.Index("ix_census_partition_progress", "census_id", "status", "partition_id"),
)

acquisition_census_candidate = sa.Table(
    "acquisition_census_candidate", METADATA,
    ...
    sa.Index("ix_census_candidate_status", "census_id", "status"),
)

acquisition_census_identity = sa.Table(
    "acquisition_census_identity", METADATA,
    ...
    sa.Index("ix_census_identity_candidate", "candidate_id"),
)

acquisition_census_occurrence = sa.Table(
    "acquisition_census_occurrence", METADATA,
...
acquisition_census_sample_page = sa.Table(
    "acquisition_census_sample_page", METADATA,
    sa.Column("plan_id", sa.String(64), sa.ForeignKey("acquisition_census_sample_plan.plan_id", ondelete="RESTRICT"), primary_key=True),
    sa.Column("partition_id", sa.String(64), sa.ForeignKey("acquisition_census_partition.partition_id", ondelete="RESTRICT"), primary_key=True),
...
# B1 freezes its page and budget allocation. New candidates discovered on a
# frozen page are added only after that page's paid call is checkpointed.
acquisition_census_b1_plan = sa.Table(
    "acquisition_census_b1_plan", METADATA,
    sa.Column("plan_id", sa.String(64), primary_key=True),
    sa.Column("census_id", sa.String(64), sa.ForeignKey("acquisition_census_run.census_id", ondelete="RESTRICT"), nullable=False),
    sa.Column("plan_hash", sa.String(64), nullable=False),
    sa.Column("seed", sa.String(64), nullable=False),
    ...
    sa.Index("ix_census_b1_plan_run", "census_id"),
)

acquisition_census_b1_page = sa.Table(
    "acquisition_census_b1_page", METADATA,
    ...
    sa.Index("ix_census_b1_page_progress", "plan_id", "status"),
)

acquisition_census_b1_entry = sa.Table(
    "acquisition_census_b1_entry", METADATA,
    sa.Column("plan_id", sa.String(64), sa.ForeignKey("acquisition_census_b1_plan.plan_id", ondelete="RESTRICT"), primary_key=True),
    sa.Column("candidate_id", sa.String(64), sa.ForeignKey("acquisition_census_candidate.candidate_id", ondelete="RESTRICT"), primary_key=True),
    sa.Column("selection_rank", sa.Integer, nullable=False),
    sa.Column("stratum", sa.JSON, nullable=False),
    ...
    sa.Index("ix_census_b1_entry_progress", "plan_id", "status", "selection_rank"),
)

acquisition_census_b1_decision = sa.Table(
    "acquisition_census_b1_decision", METADATA,
    ...
    sa.Index("ix_census_b1_decision_status", "ruleset_version", "decision"),
    sa.Index("ix_census_b1_decision_history", "candidate_id", "decided_at"),
)

acquisition_census_b1_ready_lead = sa.Table(
    "acquisition_census_b1_ready_lead", METADATA,
    sa.Column("candidate_id", sa.String(64), sa.ForeignKey("acquisition_census_candidate.candidate_id", ondelete="RESTRICT"), primary_key=True),
    sa.Column("program_key", sa.String(32), nullable=False),
    ...
    sa.Index("ix_census_b1_lead_segment", "decision", "segment"),
)

acquisition_census_legal_http_attempt = sa.Table(
    "acquisition_census_legal_http_attempt", METADATA,
    ...
    sa.Index("ix_census_legal_http_run", "census_id", "request_type", "outcome"),
)
