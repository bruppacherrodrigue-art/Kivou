...
The Milo Mail ruleset emits SEND/HOLD/NO_SEND as a **theoretical** eligibility result. It is separate from SPEC-025's Kivou purpose and records score, policy version, reason codes, and evidence IDs. A SEND result cannot schedule or export a lead: the existing Policy Gateway re-evaluates executable operations, and the program's fixed SHADOW/disabled/zero-volume configuration rejects every Instantly mutation. This separation permits representative SEND tests without external delivery.
...
No French landing URL or authorized secondary sender domain is supplied. The program must be installed disabled in SHADOW with zero contact and cost limits. No live sender credentials, provider mutation, production migration, or outbound mail are part of this implementation.
