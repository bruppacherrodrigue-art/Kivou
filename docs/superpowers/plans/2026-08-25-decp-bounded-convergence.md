...
- [x] Implement a DECP runner path which processes one complete `SourceWindow` at a time, accumulates counters, saves the versioned cursor after each completed unit, and stops successfully between units at the configured quota/deadline.
- [x] Thread a stop/deadline callback through `DecpClient.fetch_contracts_since` and DECP acquisition so pagination can stop before the systemd timeout; retain idempotence when the current day must replay.
...
- [x] Write failing tests for positive `KIVOU_DECP_MAX_WINDOWS_PER_RUN`, `KIVOU_DECP_TIME_BUDGET_SECONDS`, `KIVOU_DECP_OVERLAP_DAYS`, and `KIVOU_INGESTION_STALE_RUN_SECONDS`, plus CLI overrides.
...
- [x] Run the full targeted DECP/ingestion matrix, Ruff on changed Python, `systemd-analyze verify` on both units, and `git diff --check`.
