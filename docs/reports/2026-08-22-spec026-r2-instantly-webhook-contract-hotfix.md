# SPEC-026-R2/R2.1 — Instantly V2 webhook contract hotfix

Date: 2026-08-22
Authoritative base: `98503080f1cf7bb1a11741fbed7843a2b41944c6`
Implementation branch: `fix/spec026-instantly-v2-webhooks`
Draft PR: #43
Original R2 executable SHA: `78e301defdc5a7476c8b5dedb5483fde5300dce8`
Original R2 executable CI: `32567892544` — SUCCESS
R2.1 executable SHA: `9f24665cf14db1963ac5dc88f409f7f40424b988`
R2.1 executable CI: `32571226341` — SUCCESS
...
SPEC-026-R2/R2.1 adds no Email API method, scope, key, request, arbitrary Unibox URL fetch, reply-content persistence, or response classification. `unibox_url` is accepted only as transient provider compatibility data and may contribute to dedupe; it is never fetched, persisted, exposed to Hermes, or treated as an authenticated object identity. Webhook reply content is only a transport identity input in SPEC-026; the typed Email V2 handoff remains the planned durable response-content boundary for SPEC-027.

## Fingerprint and PII boundary

The canonical event fingerprint remains `provider-event-fingerprint-v2`; R2 was not merged before this correction, so no v3 split was introduced. A domain-separated keyed HMAC binds the available normalized transport tuple: event type, `workspace`, campaign ID, optional email event ID, timestamp, optional step/variant, and transient normalized lead email, sending account, and Unibox URL where present. For `reply_received` and `auto_reply_received`, any present documented reply subject/snippet/text/HTML values also enter the canonical HMAC input. Key rotation still checks every retained key version. Only the final HMAC, fingerprint version, and key version are persisted; no standalone reply-content digest exists.

Identical reply content plus identical normalized transport metadata converges to one event. Different documented reply content with the same workspace, campaign, member, event type, step, timestamp, and Unibox URL produces a different final HMAC and a second PII-minimized provider-event row. Bodyless reply deliveries still use the existing bounded transport tuple and converge on redelivery. Arbitrary unknown provider enrichment remains excluded from identity. A distinct second reply received after the member is already STOPPED remains real transport evidence, while sequence state and deterministic PAUSE operations stay idempotent.

Raw lead email, sending account, campaign name, Unibox URL, person/company/phone enrichment, subject/body/HTML, reply content, raw JSON, API key, and webhook secret do not enter campaign/member rows, provider-operation rows, `acquisition_provider_event`, acquisition event payloads, Policy arguments, repr output, or error strings. The provider-event row may persist the already-bound provider lead ID from Kivou's campaign member; it never persists the raw email used transiently to resolve that member.
...
R2.1 validation at executable SHA `9f24665cf14db1963ac5dc88f409f7f40424b988`:

- focused webhook suite: 35 passed;
- complete Campaign Factory and Instantly adapter suite: 164 passed;
- full backend: 3,746 passed, 2 skipped;
- skipped tests: exactly the two existing opt-in Stripe TEST smokes in `tests/test_billing_stripe_test_smoke.py`; no SPEC-026 test is skipped;
- Ruff: PASS;
- frontend: 253 passed;
- frontend build: PASS;
...
- frontend lint: PASS;
- `git diff --check`: PASS;
- executable CI `32571226341`: SUCCESS with backend 3,746/2 and frontend 253, all gates green.
