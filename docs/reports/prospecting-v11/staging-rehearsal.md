...
[rehearsal-checks.py](rehearsal-checks.py) est un helper de **contrôles uniquement** : ni dump, ni CREATE/DROP, ni bascule de liens. `tests/test_staging_rehearsal_checks.py` : **35/35 réussis en 54,40 s, sans skip**, SQLite et PostgreSQL jetable local ; Ruff et compilation Python verts. Ces tests emploient des données synthétiques et un lecteur BOAMP injecté. Ils ne prouvent ni restauration complète de staging, ni couverture des quatre avis publics réels. La concurrence identité est validée séparément : six cas ciblés puis 85/85 sans skip, références dans [qa-matrix.md](qa-matrix.md).
...
Les sections 3–5 décrivent les assertions du helper et la revue métier complémentaire ; ne pas lancer en plus une reprise CLI indépendante. Le helper imprime uniquement un JSON agrégé : code 0 `passed`, code 2 `failed` avec motif fermé, code 3 `coverage_incomplete`. Un succès du helper reste insuffisant sans les contrôles opérateur de restauration, repli, nettoyage et sources réelles. Le gate backend global demeure incomplet ; L13/L14 ne sont pas validés par cette preuve locale.
...
- [ ] **Avant migration**, réfléchir les tables de la copie. Partir des tables possédant `account_id`, puis inclure transitivement leurs descendants par clés étrangères : `auth_session` et `password_reset` sont ainsi couverts via `auth_user`. Conserver en mémoire toutes les lignes indexées par clé primaire, avec les colonnes exactes d'origine, notamment compte/profil, notes, feedback, contacts, notifications et facturation. Aucun corps ni hash de note n'est publié.
- [ ] Borne de sécurité du helper : au maximum 10 000 lignes par table, lecture avec `LIMIT 10001`, et 256MiB cumulés. Si dépassée, arrêter la répétition et vérifier une borne adaptée ; ne jamais afficher « conservé » après un échantillon partiel.
...
- [ ] Relire chaque clé primaire inventoriée et comparer **toutes ses colonnes d'origine** à la valeur d'avant migration. Les colonnes nouvelles telles que `revision` ne font pas partie de cette égalité. Exiger également que chaque ancienne clé existe encore.
