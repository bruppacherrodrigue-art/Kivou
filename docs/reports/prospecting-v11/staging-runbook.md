...
Répéter le fragment fermé dans une configuration nginx isolée : PUT note/statut/contact, POST lookup et PATCH ICP → 503 ; GET, authentification et webhook restent routés. Le test ne contacte aucun fournisseur. Le rollback reste protégé tant que ce fragment fermé est conservé, y compris si l'ancienne application ne le connaît pas.
...
2. Stopper le timer `kivou-notice-retention.timer` tant que l'ancien exécutable n'a pas le module. Ne pas supprimer les unités ou archives pour autant.
...
5. Déployer un correctif compatible 0060, recette notes/statuts/contacts, puis seulement tester et publier le fragment ouvert et réactiver le timer. Les données saisies sur V11 redeviennent disponibles.

Consigner temps, SHA, tests et état du garde-fou ; ne jamais qualifier de rollback réussi une ancienne interface qui peut écraser des révisions nouvelles.
