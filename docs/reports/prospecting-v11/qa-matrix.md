...
| B1 | Gate backend complet sur476 : **6 438 réussis, 11 skips, un xfail**, aucun échec inattendu, code0 en2 644,81s. Complément conflits SQLite/PG : **22/22**, sans skip, en2,22s. | Logs `/tmp/kivou-v11-backend-release-gate-2.log` et `/tmp/kivou-v11-persistence-conflicts-pg-complement.log` relus. Tests lents inclus ; le complément configure le troisième alias PG et exerce les neuf variantes initialement ignorées. Deux smokeStripe restent opt-in ; xfail strict historique de frontière HTTP `companies/france.py`. Fixtures historiques réparées depuis leurs véritables révisions, 76/76 ciblés avant le full. Les anciens runs interrompus/rouges ne sont plus les preuves finales. |
| CI1 | CI complète sur476 : quatre shards backend, frontend et décision **success**. | [Run34777544050](https://github.com/bruppacherrodrigue-art/Kivou/actions/runs/34777544050), terminé2026-09-13 à19:53UTC ; PR251 vers base staging dédiée, aucune fusion main/production. Le delta ultérieur du helper exige ses propres tests et une nouvelle CI candidate. |
...
| B4 | Helper streaming : **54/54**, sans skip, SQLite/PostgreSQL ;75,43s isolé puis **105,73s intégré**, code0. Ruff/compilation verts. | `/tmp/kivou-v11-streaming-helper-integrated.log`, commande C4. Inventaire exhaustif PK+HMAC privé par lots, test10017descendants, dernier lot altéré/supprimé détecté, ajouts permis, bornes/cleanup ; notes2000/CAS/tombstones/statuts/export anciens exacts, quatre avis/reprise max3/idempotence. Retour `contacted→new` conserve `contacted_at` et son export. Remplace35/35 du helper en mémoire. Données synthétiques/lecteur BOAMP injecté, pas une restauration staging. |
...
Preuves connues : B2/B3 : migrations/audit et concurrence SQLite+PostgreSQL ; B4 : helper54/54 incluant baseline streaming exhaustive, préservation des comptes, sessions/réinitialisations indirectes, notes, retour `contacted→new` sans perte de l'historique et export ; O1 : nginx isolé sur staging, dix routes vérifiées. Cela ne prouve pas encore la répétition sur restauration complète ni une bascule/restauration réelle des symlinks.
