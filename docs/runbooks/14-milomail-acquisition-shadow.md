...
An Apollo verified business email, a matching active company domain, an allowed director role, a professional source URL/type and fresh observation are all required to confirm recipient capacity. Active-company status must come from a separately dated public/company-registry proof; Apollo listing does not imply an active company. Public activity and service type contribute only documented score proxies. No Gmail content, inbox volume, OAuth token, private metadata, sensitive category, or invented pain statistic enters Kivou.
