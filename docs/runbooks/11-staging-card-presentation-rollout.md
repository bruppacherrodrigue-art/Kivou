...
Revalider le rollback target et les blocs de la procédure depuis la release
elle-même, immédiatement avant le blue/green :
...
test "$(sudo -u kivou git -C "$KIVOU_RELEASE_DIR" rev-parse HEAD)" = \
  "$KIVOU_FINAL_SHA"
KIVOU_BLUE_GREEN_DOC="$KIVOU_RELEASE_DIR/ops/README.md"
test -f "$KIVOU_BLUE_GREEN_DOC"
for KIVOU_HEADING in \
  '### Démarrer et prouver le runtime vert sur 8001' \
  '### Publier le bundle sûr puis effectuer le single public reload to green' \
  "### Basculer l'application pendant le monitor public"; do
  grep --fixed-strings --line-regexp --quiet "$KIVOU_HEADING" \
    "$KIVOU_BLUE_GREEN_DOC"
done
REMOTE
~~~

La procédure autoritaire est la section **`Reverse proxy public de staging
(#84)`** de [`ops/README.md`](../../ops/README.md). L'étape 4 a déjà exécuté la
création de release du premier bloc : ne pas créer une deuxième release. Dans
le même shell opérateur, rétablir les variables non secrètes validées et
**reprendre au second bloc bash**, celui qui crée le candidat nginx, puis
exécuter sans raccourcir ni réordonner tous les blocs suivants :

1. créer le candidat nginx isolé et exécuter son `nginx -t`;
2. `Capturer la preuve antérieure sans en faire un rollback`, en exigeant que
   la previous release soit `KIVOU_PREVIOUS_BACKEND`;
3. `Démarrer et prouver le runtime vert sur 8001`, y compris
   `kivou-api-green.service 8001`, puis conserver
   `green_openapi_status=200` et `green_me_status=401`;
4. publier le bundle sûr et effectuer l'unique reload public vers green;
5. démarrer le monitor public, exécuter exactement
   `sudo mv -Tf "$KIVOU_APP_NEXT" /srv/kivou/app`, installer l'unité versionnée,
   prouver 8000 et remettre le proxy sur 8000;
6. exiger que chaque ligne de `public-status.codes` soit `200 401` avant
   d'arrêter green.

STOP si le document de la release ne contient pas ces blocs ou si sa topologie
diffère du preflight. La migration 0028 doit déjà être verte avant la première
commande de démarrage green; cette procédure ne migre rien.
...
KIVOU_ASSET_PATH=$(grep -Eo '/assets/[^"[:space:]]+' \
  "$KIVOU_FRONTEND_RELEASE/index.html" | head -n 1)
case "$KIVOU_ASSET_PATH" in (/assets/*) ;; (*) exit 69 ;; esac
test -f "$KIVOU_FRONTEND_RELEASE$KIVOU_ASSET_PATH"

kivou_frontend_http_smoke() {
  KIVOU_SMOKE_ASSET=$1
  case "$KIVOU_SMOKE_ASSET" in (/assets/*) ;; (*) return 1 ;; esac
  for KIVOU_PATH in / /app/dashboard /app/companies /app/signals "$KIVOU_SMOKE_ASSET"; do
...
KIVOU_FRONTEND_NEXT=/srv/kivou/frontend.next
sudo test ! -e "$KIVOU_FRONTEND_NEXT"
sudo test ! -L "$KIVOU_FRONTEND_NEXT"
sudo ln -s "$KIVOU_FRONTEND_RELEASE" "$KIVOU_FRONTEND_NEXT"
sudo chown -h kivou:kivou "$KIVOU_FRONTEND_NEXT"
test "$(sudo readlink -f "$KIVOU_FRONTEND_NEXT")" = "$KIVOU_FRONTEND_RELEASE"
sudo mv -Tf "$KIVOU_FRONTEND_NEXT" /srv/kivou/frontend
if ! kivou_frontend_http_smoke "$KIVOU_ASSET_PATH"; then
  KIVOU_FRONTEND_ROLLBACK=/srv/kivou/frontend.rollback
  sudo test ! -e "$KIVOU_FRONTEND_ROLLBACK"
  ...
  KIVOU_ROLLBACK_ASSET_PATH=$(grep -Eo '/assets/[^"[:space:]]+' \
    "$KIVOU_PREVIOUS_FRONTEND/index.html" | head -n 1)
  case "$KIVOU_ROLLBACK_ASSET_PATH" in (/assets/*) ;; (*) exit 69 ;; esac
  test -f "$KIVOU_PREVIOUS_FRONTEND$KIVOU_ROLLBACK_ASSET_PATH"
...
with engine.connect() as connection:
    ...
    active_users = connection.scalar(sa.text(
        "SELECT count(*) FROM auth_user WHERE account_id=:account_id AND is_active"
    ), {"account_id": account_id})
    ...
    current_signals = connection.scalar(sa.text(
        ...
        "JOIN target_icp AS icp ON icp.target_icp_id=signal.target_icp_id "
        "WHERE icp.account_id=:account_id AND icp.status='active' "
        "AND icp.plan_limited_at IS NULL AND signal.invalidated_at IS NULL "
...
print(
    f"qa_account_fingerprint={fingerprint} active_users={active_users} "
...
with engine.connect() as connection:
    rows = connection.execute(sa.text(
        "SELECT language, qa_status, payload_variant, payload, provider, model_id, "
        "prompt_version, qa_provider, qa_model_id FROM card_presentation_artifact "
    ...
    foreign_rows = connection.scalar(sa.text(
        "SELECT count(*) FROM card_presentation_artifact WHERE account_id<>:account_id"
    ), {"account_id": account_id})
    duplicates = connection.scalar(sa.text(
        "SELECT count(*) FROM (SELECT account_id,signal_key,target_icp_id,artifact_kind,language "
        "FROM card_presentation_artifact WHERE published_at IS NOT NULL "
        "AND superseded_at IS NULL GROUP BY 1,2,3,4,5 HAVING count(*)>1) AS duplicate"
    ))
assert rows
...
for row in rows:
    ...
    assert all(row[field] is None for field in (
        "provider", "model_id", "prompt_version", "qa_provider", "qa_model_id"
    ))
    ...
    assert payload.claims and all(claim.evidence_refs for claim in payload.claims)
counts = {language: sum(row["language"] == language for row in rows) for language in ("fr", "en")}
print(f"factual_rows_fr={counts['fr']} factual_rows_en={counts['en']} ai_enabled=0")
PY
REMOTE
~~~
...
~~~bash
ssh kivou-staging 'bash -s' -- "$KIVOU_PREVIOUS_FRONTEND" <<'REMOTE'
set -euo pipefail
...
sudo mv -Tf "$KIVOU_FRONTEND_ROLLBACK" /srv/kivou/frontend
test "$(readlink -f /srv/kivou/frontend)" = "$KIVOU_PREVIOUS_FRONTEND"
KIVOU_PREVIOUS_ASSET_PATH=$(grep -Eo '/assets/[^"[:space:]]+' \
  "$KIVOU_PREVIOUS_FRONTEND/index.html" | head -n 1)
case "$KIVOU_PREVIOUS_ASSET_PATH" in (/assets/*) ;; (*) exit 69 ;; esac
test -f "$KIVOU_PREVIOUS_FRONTEND$KIVOU_PREVIOUS_ASSET_PATH"
for KIVOU_PATH in / /app/dashboard /app/companies /app/signals \
  "$KIVOU_PREVIOUS_ASSET_PATH"; do
  test "$(curl --silent --show-error --connect-timeout 2 --max-time 5 \
    --output /dev/null --write-out '%{http_code}' \
    --resolve staging.kivou.eu:443:127.0.0.1 \
    "https://staging.kivou.eu$KIVOU_PATH")" = 200
...
Pour le backend, exécuter intégralement `Rollback applicatif préservant la
sécurité` dans la section `Reverse proxy public de staging (#84)` de
`ops/README.md`. Cette reprise démarre la release de sécurité sur green, prouve
8001, republie le bundle sûr sous monitor, puis commute `/srv/kivou/app` vers
le `KIVOU_PREVIOUS_BACKEND` capturé — jamais vers un glob.
