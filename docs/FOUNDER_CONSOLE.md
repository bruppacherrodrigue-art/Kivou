...
There is no permanent `control.staging.kivou.eu` console. Validation happens in tests, CI and local/ephemeral review before deployment. When production is ready, this single console is connected directly to production read models.
...
7. PR 2 uses a PostgreSQL role with `default_transaction_read_only=on` and no write grants.
...
src/signals/founder_api/          separate FastAPI application
...
- filter or change a reporting period;
- open evidence and internal references;
- display incidents, dead letters, quality signals and commercial metrics.
...
The deployment copies `frontend/dist-founder/` atomically to `/srv/kivou-founder/frontend/` and installs the versioned systemd/nginx files through a candidate + validation + rollback procedure.
...
- Today / Attention / Business / Quality / System views;
