...
export function SignalDetail({ signalKey, onClose }: { signalKey: string; onClose: () => void }) {
  ...
  return <DetailFrame title={fr ? 'Détail du signal' : 'Signal details'} onClose={onClose}
    ...
    {upgrade && <UpgradeDialog onClose={() => setUpgrade(false)} intent={{ kind: 'signal', signalKey, ...(artifact ? { artifactId: artifact } : {}) }} upgradeTo={detail?.locked ? detail.access.upgrade_to : undefined} />}
  </DetailFrame>
}
