...
export function SignalDetail({ signalKey, onClose }: { signalKey: string; onClose: () => void }) {
  ...
  return <DetailFrame title={fr ? 'Détail du signal' : 'Signal details'} onClose={onClose}
    ...
    {detail?.locked && <section className={styles.empty}><h2>{fr ? 'Un marché pour votre prospection' : 'A contract for your prospecting'}</h2><p>{detail.headline}</p>{detail.landing_example_holder && <p>{fr ? `Comme pour ${detail.landing_example_holder} : dirigeant, téléphone, e-mail, historique des marchés.` : `As with ${detail.landing_example_holder}: director, phone, email and contract history.`}</p>}<button onClick={() => setUpgrade(true)} className={styles.primary}>{detail.landing_example_holder ? (fr ? 'Voir ce contact — 49 €/mois' : 'View this contact — €49/month') : (fr ? 'Voir mes possibilités d’accès' : 'View access options')}</button></section>}
    {item && <SignalContent item={item} onUpgrade={() => setUpgrade(true)} holder={<HolderSummary name={item.company.name ?? (fr ? 'Titulaire du marché' : 'Contract holder')} href={item.landing_demo ? null : companyHref}
