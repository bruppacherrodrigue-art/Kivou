...
export function SignalListRow({ item, onOpen }: { item: FeedItem; onOpen: () => void }) {
  ...
  return <article className={styles.signalRow} data-signal-id={item.signal_id}>
    ...
    <div className={styles.rowMain}>
      ...
      <div className={styles.rowMeta}>{!item.locked && item.company.name && <strong>{item.company.name}</strong>}{!item.locked && item.company.consortium && <span className={styles.tag}>{fr ? 'Groupement' : 'Consortium'}</span>}<span>{item.locked ? visiblePlaceName(item.teaser.department) : signalPlace(item)}</span>{!item.locked && <span>{item.analysis.fit.label}</span>}</div>
    ...
    <div className={styles.rowAside}>
      {money && <span className={styles.money}>{formatAmount(money, locale)}</span>}
