...
import type { CompanyProfile as CompanyProfilePayload, Place } from '../api/types'
...
export function CompanyProfileView({
  ...
  const publishedAwardDate = date(signal.awardDate)
  const observedDate = date(signal.eventDate)
  const dateStatement = publishedAwardDate
    ? interpolate(copy.awardedOn, { date: publishedAwardDate })
    : observedDate
      ? interpolate(copy.eventRecordedOn, { date: observedDate })
    ...
    >
      ...
      <div className={styles.detailSections}>
        <section className={styles.contextCard} aria-labelledby="selected-award-title">
          ...
          <article className="company-timeline-item">
            ...
            <div>
              ...
              <p>{publishedAmount} · {signal.whyNow}</p>
            </div>
            <ReferenceLink
              dashboard
        ...
        <section className={styles.contextCard} aria-labelledby="match-title">
          ...
          {signal.matchReasons.length > 0 ? (
            <ul className={styles.reasonList}>
              {signal.matchReasons.map((reason, index) => <li key={`${reason}-${index}`}>{reason}</li>)}
            </ul>
          ) : <p>{copy.matchMissing}</p>}
          {signal.plausibleNeed ? (
            <p className={styles.inference}><strong>{copy.plausibleNeed}</strong> {signal.plausibleNeed}</p>
          ) : null}
        </section>
        ...
        <section className="company-roles-card" aria-labelledby="action-title">
          ...
          <p>{copy.actionFallback}</p>
          <ul className="company-roles-list" aria-label={t.reference.headings.usefulRoles}>
        ...
        <section className="related-signals-card" aria-labelledby="related-title">
          ...
          {otherSignals.length > 0 ? otherSignals.map((candidate) => {
            const candidateDate = date(candidate.awardDate ?? candidate.eventDate) ?? copy.awardDateMissing
            ...
            return (
              <article className="company-timeline-item" key={candidate.signalId}>
                ...
                <div>
                  ...
                  <p>{candidateAmount} · {candidate.whyNow}</p>
                </div>
                <Link
