...
export function SignalsFeed() {
  ...
  return <main className={styles.workspace} data-page="signals">
    ...
    <section className={styles.panel} aria-label={fr ? 'Liste des signaux' : 'Signal list'}>
      ...
      {data && <div className={styles.resultMeta}><span>{number(data.items.length)} {fr ? data.items.length === 1 ? 'signal sur cette page' : 'signaux sur cette page' : data.items.length === 1 ? 'signal on this page' : 'signals on this page'}</span>{data.history_access?.scope === 'grants_only' && <span>{fr ? 'Votre sélection Découverte' : 'Your Discovery selection'}</span>}{data.page.scan_truncated && <span>{fr ? 'Affinez votre recherche pour explorer davantage de résultats.' : 'Refine your search to explore more results.'}</span>}</div>}
