...
export function SignalsFeed() {
  ...
  return <main className={styles.workspace} data-page="signals">
    ...
    <section className={styles.panel} aria-label={fr ? 'Liste des signaux' : 'Signal list'}>
      ...
      {data && <div className={styles.resultMeta}><span>{fr ? `${number(profileTotal)} ${profileTotal === 1 ? 'marché attribué correspondant' : 'marchés attribués correspondant'} à votre profil ces 30 jours` : `${number(profileTotal)} awarded ${profileTotal === 1 ? 'contract' : 'contracts'} matching your profile in the last 30 days`}</span>{discovery && <span>{number(discovery.granted_signal_count)}/{number(discovery.limit)} {fr ? 'signaux ce mois' : 'signals this month'}</span>}{data.page.scan_truncated && <span>{fr ? 'Affinez votre recherche pour explorer davantage de résultats.' : 'Refine your search to explore more results.'}</span>}</div>}
