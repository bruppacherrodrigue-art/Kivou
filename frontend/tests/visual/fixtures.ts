...
function toUnlockedItem(record: AwardSignal): UnlockedFeedItem {
  ...
  return {
    ...
    presentation: {
      artifact_id: `card_${record.id}_v1`,
      schema_version: 'card-presentation-v1',
      version: 1,
      status: 'PASS',
      ...
      content: {
        ...
        award_summary: record.summary,
        commercial_importance: record.matchReasons[0]?.value ?? record.summary,
        fit_reason: record.matchLabel,
        timing: record.matchReasons[2]?.value ?? record.execution,
        recommended_action: record.questionsToVerify[0] ?? record.limit,
        target_roles: ['SITE_PROCUREMENT_MANAGER'],
        fit_need_categories: ['materials_or_components'],
        unknowns: record.questionsToVerify.slice(0, 3),
        claims: [
          {
            claim_id: 'AWARD_FACT',
            kind: 'FACT',
          ...
          {
            claim_id: 'FIT_INFERENCE',
            kind: 'INFERENCE',
            text: record.matchReasons[0]?.value ?? record.matchLabel,
            evidence_refs: [`notice:${record.notice}`],
            confidence: 'medium',
          },
          {
            ...
            kind: 'RECOMMENDATION',
            text: record.questionsToVerify[0] ?? record.limit,
            evidence_refs: [`notice:${record.notice}`],
            confidence: null,
          },
        ],
