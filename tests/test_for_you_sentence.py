...
def test_fallback_and_legacy_cache_never_expose_engine_vocabulary() -> None:
    assert fallback_sentence(context()) == (
        "Rénovation d'une école à Grenoble à Grenoble, Isère (250 k€, août 2026) : "
        "dans votre zone et votre secteur."
    )
    assert client_safe_sentence("besoin principal couvert : materials_or_components") is None
    assert (
        client_safe_sentence(
            "Le marché « Travaux de voirie » à Isère (250000 EUR, attribué le 2026-09-01) "
            "peut concerner votre activité."
        )
        is None
    )
