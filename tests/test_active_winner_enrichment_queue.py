...
def test_inactive_winner_queue_cleanup_is_dry_and_pending_only(tmp_path) -> None:
    ...
    with engine.begin() as connection:
        ...
        inactive_account = make_account(connection, "inactive-holder@example.test", "Inactive")
        inactive_icp = make_icp(connection, inactive_account)
        inactive = materialize_simap(connection, "29997-02", target_icp_id=inactive_icp)
        connection.execute(
    ...
    with engine.connect() as connection:
        dry_jobs = connection.scalar(sa.select(sa.func.count()).select_from(winner_enrichment_job))
    ...
    with engine.connect() as connection:
        remaining = tuple(connection.scalars(sa.select(winner_enrichment_job.c.signal_key)))
...
def test_cleanup_never_deletes_non_pending_history(tmp_path) -> None:
    ...
    with engine.begin() as connection:
        ...
        connection.execute(
            sa.update(target_icp).where(target_icp.c.target_icp_id == icp_id).values(status="draft")
        )
        connection.execute(
...
def test_holder_shared_by_active_and_inactive_accounts_is_not_a_purge_candidate(
    tmp_path,
) -> None:
    engine = _engine(tmp_path)
    with engine.begin() as connection:
        active_account = make_account(connection, "shared-active@example.test", "Active")
        active_icp = make_icp(connection, active_account)
        active = materialize_simap(connection, "33112-02", target_icp_id=active_icp)

        inactive_account = make_account(connection, "shared-inactive@example.test", "Inactive")
        inactive_icp = make_icp(connection, inactive_account)
        inactive = materialize_simap(connection, "29997-02", target_icp_id=inactive_icp)
        connection.execute(
            sa.update(target_icp)
        ...
        connection.execute(
            ...
            .where(materialized_signal.c.signal_key.in_((active.signal_key, inactive.signal_key)))
            .values(winner_identifier_value="123456789")
        )

    report = maintain_active_winner_queue(engine, apply=True)

    with engine.connect() as connection:
        remaining = set(connection.scalars(sa.select(winner_enrichment_job.c.signal_key)))
    assert report.inactive_purge_candidates == 0
    assert report.active_distinct_holders == 1
    assert remaining == {active.signal_key, inactive.signal_key}
